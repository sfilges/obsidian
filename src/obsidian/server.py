...
def search_notes(query: str, limit: int = 5) -> str:
    ...
    table = get_table()
    if table is None:
        return "Error: Database not initialized. Please run 'obsidian lance' first to ingest your notes."

    # We select the fields we want to return to Claude
    results = (
        table.search(query_vector)
        .limit(limit)
        .select(["title", "content", "filename", "relative_path", "created_date", "note_type"])
        .to_list()
    )
...
def read_full_note(filename: str) -> str:
    ...
    try:
        table = get_table()
        if table is None:
            return "Error: Database not initialized. Please run 'obsidian lance' first."
